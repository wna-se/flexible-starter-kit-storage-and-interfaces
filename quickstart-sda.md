...
The storage and interfaces stack can be deployed with the use of the provided `docker-compose-example.yml` file by running
...
Configuration can be further customized by changing the files listed in `./.env` along with the `docker-compose-example.yml` itself. Please bear in mind that environment variables take precedence over the `config.yaml` file.
...
where `<sda-c4gh-public-key>` is the archive's public key. Note that `docker-compose-example.yml` stores the archive's c4gh public key in a volume named `shared`, see [below](#how-to-perform-common-admin-tasks) for how to extract it.
...
```shell
docker compose -f docker-compose-example.yml logs <container-name> -f
```
